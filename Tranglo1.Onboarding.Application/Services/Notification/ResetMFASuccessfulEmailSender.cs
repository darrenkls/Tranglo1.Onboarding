...
{
    public sealed class ResetMFASuccessfulEmailSender
    {
        private readonly TrangloUserManager _userManager;
        private readonly IBusinessProfileRepository _businessProfileRepository;
        private readonly INotificationService _notificationService;
        private readonly ILogger<ResetMFASuccessfulEmailSender> _logger;

        public ResetMFASuccessfulEmailSender(TrangloUserManager userManager,
            IBusinessProfileRepository businessProfileRepository,
            INotificationService notificationService,
            ILogger<ResetMFASuccessfulEmailSender> logger)
        {
        ...
        {
            #region Send Reset 2FA Successful Email for Customer
            if (user is CustomerUser customerUser)
            {
                ...
                foreach (var solutionGroup in partnerSubscriptionsGroupedBySolutionCode)
                {
                    ...
                    {
                        return Result.Failure("Reset two-factor authentication (2FA) is completed. " +
                            "Fail to send the successful two-factor authentication (2FA) reset email. Please contact the support.");
                    }
                }
            }
            #endregion
            #region Send Reset 2FA Successful Email for Non-Customer
            else
            {
        ...
        private async Task<Result> SendEmailAsync(ApplicationUser user,
            ...
            string entityName)
        {
            NotificationTemplate notificationTemplate = NotificationTemplate.Reset2faSuccessful;
            string subject = "Successful two-factor authentication (2FA) reset";
            ...
            EmailNotificationInputDTO emailNotificationInputDTO = new EmailNotificationInputDTO
            {
                ...
                SubModule = "Successfully Reset 2FA",
            ...
            {
                _logger.LogError("Sending Email for two-factor authentication (2FA) reset is FAILED! [{Error}]", sendEmailResponse.Error);
                return Result.Failure("Fail to send the two-factor authentication (2FA) reset email. Please contact the support.");
            }
